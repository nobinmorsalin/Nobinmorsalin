...
(function () {
  ...
  function renderWorkflow() {
    ...
    const workflow = getData('workflow');
  ...
  function renderStats() {
    ...
    const stats = getData('settings')?.stats || {};
    ...
    const statValues = {
      services: stats.services ?? (Array.isArray(services) ? services.length : 0),
      projects: stats.projects ?? (Array.isArray(projects) ? projects.length : 0),
      clients: stats.clients ?? (Array.isArray(clients) ? clients.length : 0),
    };

    $$('[data-stat="services"]').forEach(el => {
      el.textContent = statValues.services;
    });

    $$('[data-stat="projects"]').forEach(el => {
      el.textContent = statValues.projects;
    });

    $$('[data-stat="clients"]').forEach(el => {
      el.textContent = statValues.clients;
    });
  }
