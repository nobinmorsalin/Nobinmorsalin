/* NOBIN MORSALIN — PREMIUM UX LAYER v1.1
...
(function(){'use strict';
  ...
  function scrollProgress(){
    let bar=$('.premium-scroll-progress');
    if(!bar){bar=document.createElement('div');bar.className='premium-scroll-progress';document.body.appendChild(bar)}
    let back=$('.premium-backtop');
    if(!back){back=document.createElement('button');back.className='premium-backtop';back.type='button';back.setAttribute('aria-label','Back to top');back.innerHTML='↑';document.body.appendChild(back);back.addEventListener('click',()=>scrollTo({top:0,behavior:reduce?'auto':'smooth'}))}
  ...
  function prepareRevealElement(el,delay){
    if(!el||el.dataset.premiumPrepared==='1')return;
    el.dataset.premiumPrepared='1';el.classList.add('premium-reveal');el.dataset.revealDelay=String(delay||0);
  }

  function prepareStagger(el){
    if(!el||el.dataset.premiumStagger==='1')return;
    el.dataset.premiumStagger='1';el.classList.add('premium-stagger');
  }

  function reveal(){
    $$('.section').forEach((section,i)=>{
      prepareRevealElement(section,Math.min(i%4,3));
      const grid=section.querySelector('.services-track,.projects-grid,.workflow-steps,.skills-grid,.contact-grid');
      if(grid)prepareStagger(grid);
    });
    const elements=$$('.premium-reveal,.premium-stagger');
    ...
    /* Existing renderers can populate API/data-driven cards after this script starts. */
    const observer=new MutationObserver(mutations=>{
      let changed=false;
      mutations.forEach(m=>{if(m.type==='childList'&&m.addedNodes.length)changed=true});
      if(!changed)return;
      $$('.section').forEach((section,i)=>{prepareRevealElement(section,Math.min(i%4,3));const grid=section.querySelector('.services-track,.projects-grid,.workflow-steps,.skills-grid,.contact-grid');if(grid)prepareStagger(grid)});
      $$('.premium-reveal,.premium-stagger').forEach(el=>{if(!el.classList.contains('is-visible'))io.observe(el)});
      if(!reduce)pointerGlow();
    });
  ...
  function activeSection(){
    ...
    const io=new IntersectionObserver(entries=>entries.forEach(entry=>{
      if(!entry.isIntersecting)return;links.forEach(a=>a.classList.remove('is-current'));const link=links.find(a=>a.getAttribute('href')==='#'+entry.target.id);if(link)link.classList.add('is-current')
    }),{rootMargin:'-38% 0px -52% 0px',threshold:0});sections.forEach(s=>io.observe(s));
  }

  function pointerGlow(){
    if(reduce||!matchMedia('(hover:hover)').matches)return;
    $$('.service-card,.project-card,.workflow-step,.client-card').forEach(card=>{
      if(card.dataset.pointerGlow==='1')return;card.dataset.pointerGlow='1';
  ...
  function mobilePolish(){
    if(!mobile())return;
    const chat=$('#liveChat');if(chat)chat.style.setProperty('--mobile-safe-bottom','env(safe-area-inset-bottom)');
  }

  function init(){uxWriting();scrollProgress();reveal();activeSection();pointerGlow();addSectionMarkers();mobilePolish();addEventListener('resize',mobilePolish,{passive:true})}
