...
(() => {
  ...
  const STYLE_ID = 'premium-client-presentation-v1';
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #clients .client-card.premium-client-card {
        ...
        border-radius: 22px;
        background:
          radial-gradient(circle at 85% 10%, rgba(0,255,196,.09), transparent 34%),
          linear-gradient(145deg, rgba(18,25,31,.96), rgba(8,12,17,.96));
      ...
      #clients .client-card.premium-client-card:focus-visible {
        ...
        border-color: rgba(0,255,196,.34);
      ...
      #clients .client-card.premium-client-card .client-visual::before {
        ...
        height: 110px;
        border-radius: 50%;
        background: radial-gradient(circle, rgba(255,255,255,.08), rgba(0,255,196,.035) 44%, transparent 70%);
        filter: blur(2px);
      }
      #clients .client-card.premium-client-card .client-logo {
        ...
        filter: grayscale(1) brightness(0) invert(1) drop-shadow(0 6px 16px rgba(255,255,255,.08));
        opacity: .96;
      ...
      #clients .client-card.premium-client-card:focus-visible .client-logo {
        filter: grayscale(1) brightness(0) invert(1) drop-shadow(0 8px 20px rgba(0,255,196,.12));
        opacity: 1;
      ...
      #clients .client-card.premium-client-card .client-logo-fallback {
        ...
        border-radius: 18px;
        color: #fff;
        background: linear-gradient(145deg, rgba(255,255,255,.12), rgba(0,255,196,.08));
        border: 1px solid rgba(255,255,255,.12);
        font: 800 1.35rem/1 var(--font-head, Inter, sans-serif);
      ...
      #clients .client-card.premium-client-card .client-name {
        color: #f3f7f8;
        font: 700 1.02rem/1.25 var(--font-head, Inter, sans-serif);
        letter-spacing: -.01em;
      }
      #clients .client-card.premium-client-card .client-service {
        margin-top: 6px;
        color: rgba(220,230,233,.64);
        font: 500 .78rem/1.45 var(--font-body, Inter, sans-serif);
      }
      #clients .client-view-cue {
        display: inline-flex;
        align-items: center;
        gap: 5px;
        margin-top: 12px;
        color: rgba(0,255,196,.84);
        font: 600 .72rem/1 var(--font-body, Inter, sans-serif);
        letter-spacing: .01em;
        opacity: .82;
        transition: transform .3s ease, opacity .3s ease;
      }
      #clients .client-card.premium-client-card:hover .client-view-cue,
      #clients .client-card.premium-client-card:focus-visible .client-view-cue {
      ...
      @media (prefers-reduced-motion: reduce) {
        #clients .client-card.premium-client-card { transition: none; }
        #clients .client-card.premium-client-card .client-logo,
        #clients .client-view-cue { transition: none; }
      }
    `;
