...
(function () {
  ...
  const clientServices = (item) => {
    if (Array.isArray(item?.services)) return item.services.filter(Boolean).map(String);
    return String(item?.service || '').split(',').map((x) => x.trim()).filter(Boolean);
  };
  ...
  function open(item, type) {
    ...
    const live=url(item.live||item.liveUrl||item.url||item.demo||item.website), github=url(item.github||item.githubUrl), service=item.service||'';
    let body='';
    if(type==='client') {
      const services=clientServices(item);
      const description=item.description||item.details||item.shortDescription||'';
      body=`<div class="details-hero details-client-hero">${image?`<img src="${esc(url(image))}" alt="${esc(title)} logo" class="details-main-image details-client-image">`:`<div class="details-image-fallback">${esc(title.charAt(0))}</div>`}<div><span class="details-kicker">Client</span><h2 id="detailsTitle">${esc(title)}</h2></div></div>${description?`<div class="details-section"><h3>About This Work</h3><p class="details-lead">${esc(description)}</p></div>`:''}${services.length?`<div class="details-section"><h3>Services Delivered</h3><div class="details-tags client-service-tags">${services.map(x=>`<span>${esc(x)}</span>`).join('')}</div></div>`:''}<div class="details-grid">${item.industry?`<div><b>Industry</b><span>${esc(item.industry)}</span></div>`:''}${item.location?`<div><b>Location</b><span>${esc(item.location)}</span></div>`:''}${item.testimonial?`<div class="details-wide"><b>Testimonial</b><span>${esc(item.testimonial)}</span></div>`:''}</div>${live!=='#'?`<div class="details-actions"><a class="details-primary" href="${esc(live)}" target="_blank" rel="noopener noreferrer">Visit Website ↗</a></div>`:''}`;
    }
  ...
  function premiumCSS(){
    ...
    const s=document.createElement('style');s.id='portfolio-details-mobile-fix';s.textContent=`body.details-modal-open{overflow:hidden}.portfolio-details-dialog{-webkit-overflow-scrolling:touch;overscroll-behavior:contain}.details-client-hero{align-items:center}.details-client-image{width:160px;height:120px;object-fit:contain;object-position:center;background:rgba(255,255,255,.025);border-radius:16px;padding:14px;box-sizing:border-box}.client-card{cursor:pointer}.client-card .client-visual{height:120px;min-height:120px;display:flex;align-items:center;justify-content:center;padding:18px;box-sizing:border-box;overflow:hidden}.client-card .client-visual img{width:100%;height:100%;max-width:180px;object-fit:contain;object-position:center;display:block}.client-service-tags{display:flex;flex-wrap:wrap;gap:8px}.client-service-tags span{display:inline-flex;align-items:center;border:1px solid rgba(0,255,196,.14);background:rgba(0,255,196,.045);color:rgba(235,246,247,.82);border-radius:999px;padding:7px 10px;font-size:.74rem;line-height:1.1}.clients-track{touch-action:pan-y;will-change:transform}@media(max-width:600px){.client-card .client-visual{height:105px;min-height:105px;padding:14px}.client-card .client-visual img{max-width:150px}.details-client-image{width:120px;height:95px}}`;
