(() => {
  ...
  async function registerAdminPush() {
    ...
    if (!configResponse.ok) return { ok: false, reason: 'not-configured', detail: `push-config returned HTTP ${configResponse.status}` };
    ...
    if (!response.ok) {
      let detail = `push-subscribe returned HTTP ${response.status}`;
      try {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (result.ok) {
          ...
          alert(`Push notifications are not configured. ${result.detail || 'Check Vercel VAPID environment variables and redeploy.'}`);
          btn.disabled = false;
        } else {
