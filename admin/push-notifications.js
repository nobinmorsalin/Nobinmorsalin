(() => {
  ...
  function addButton() {
    ...
    btn.id='enablePushBtn'; btn.className='btn btn-primary'; btn.type='button'; btn.textContent='🔔 Enable Notifications';
    const status = document.createElement('span');
    status.id='pushStatus'; status.style.cssText='font-size:.78rem;opacity:.72;display:none';
    btn.addEventListener('click', async()=>{
      btn.disabled=true;
      try {
        const result=await registerAdminPush();
        if(result.ok){btn.textContent='🔔 Notifications Enabled';btn.classList.add('is-enabled');status.textContent='Push active';status.style.display='inline';}
        else if(result.reason==='permission-denied'){alert('Notification permission was denied. Please allow notifications for this site in browser settings.');btn.disabled=false;}
        else if(result.reason==='not-configured'){alert('Push notifications are not configured. Check Vercel VAPID environment variables and redeploy.');btn.disabled=false;}
        else {alert('This browser does not support push notifications.');btn.disabled=false;}
      } catch(error){console.error('ADMIN PUSH ERROR:',error);alert('Could not enable notifications. Check the browser console for details.');btn.disabled=false;}
    });
    wrap.append(btn,status); header.appendChild(wrap);
  }
  function init(){
    addButton();
    if(localStorage.getItem('adminPushEnabled')==='1') registerAdminPush().catch(()=>{});
  }
